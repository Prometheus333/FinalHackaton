...
# ---------------------------------------------------------------- LLM
# GenAI Lab is the single, only LLM provider. There is no fallback provider.
LLM_BASE_URL = os.environ.get("LLM_BASE_URL", "https://genailab.tcs.in")
LLM_API_KEY = os.environ.get("LLM_API_KEY", "sk-qebOGu50qEQ_yw76Trca5Q")
LLM_MODEL = os.environ.get("LLM_MODEL", "azure/genailab-maas-gpt-4.1")
# Low temperature + a hard output ceiling keep answers short and fast.
LLM_TEMPERATURE = _float("LLM_TEMPERATURE", 0.1)
LLM_MAX_TOKENS = _int("LLM_MAX_TOKENS", 320)
...
# Max tool iterations per agent. Kept tight: every extra iteration is a full
# round-trip to the LLM, and it is the single biggest driver of response latency.
MAX_ITERATIONS_SUBAGENT = _int("MAX_ITERATIONS_SUBAGENT", 3)
MAX_ITERATIONS_PM = _int("MAX_ITERATIONS_PM", 6)
